...
type
  ...
  TGetDatasetEvent = function(const ComplexName : string): TComponent of object;
  ...
  private
    ...
    FBandColumns, FBandCurColumn, FBandMaxY : integer;
    ...
    procedure SetBandColumns(Value : integer);
    procedure SetBandCurColumn(Value : integer);
  public
    ...
    destructor Destroy; override;
    property BandColumns : integer read FBandColumns write SetBandColumns default 1;
    property BandCurColumn : integer read FBandCurColumn write SetBandCurColumn default 1;
  end;
  ...
  private
    ...
    FOnGetDataSet : TGetDatasetEvent;
    FPageNumbers: String;
    FCopies: Integer;
    FCurPage: TfrPage;
...
{$ENDIF}
  ...
  published
    ...
    property OnGetDataSet : TGetDatasetEvent read FOnGetDataSet write FOnGetDataSet;
  end;
...
begin
  ...
  gapx := Round(FrameWidth / 2) + 2;
  gapy := Round(FrameWidth / 2) div 2 + 1;
...
begin
  ...
  HideZero := TfrMemoView(From).HideZero;
end;
...
begin
  ...
  begin
    ...
    if not IsPrinting then
      Font.Height := -Round(Font.Size * Font.PixelsPerInch {96} / 72 * ScaleY);
  end;
end;
...
begin
  ...
  WCanvas.Font.Height := -Round(Font.Size * Font.PixelsPerInch {96} / 72);
...
var
  ...
  var
    ...
    var
      ...
      ParaEnd: Boolean;
    begin
      if not Streaming or (cury + th <= DR.Bottom+1) then
      begin
        n := Length(str);
        ...
        begin
          ...
          while (aw > nw) and (Canvas.Font.Height < -1) do
          begin
            Canvas.Font.Height := Canvas.Font.Height + 1;
            aw := Canvas.TextWidth(str);
          end;
          ...
          if not Exporting then
            ExtTextOut(Canvas.Handle, curx, cury, ETO_CLIPPED, @DR,
              PChar(str), Length(str), nil{PInteger(GCP.lpDx)});
        end
        else
        begin
          curx := x + gapx;
  ...
  begin
    {if (ad and $18) = 0 then
      cury := DRect.Top + gapy
    else if (ad and $18) = $8 then
      cury  := DRect.Top + (dy - VHeight) div 2+gapy
    else if (ad and $18) = $10 then
      cury := DRect.Top + dy - VHeight+gapy;}

    cury := {DRect.Top + gapy; //}y + gapy;
...
begin
  ...
  //FCalcHeight := CalcHeight;
  VHeight := Round(FCalcHeight {VHeight} * ScaleY);
  if VHeight > dy then VHeight := dy;
  ad := Adjust;
...
begin
  ...
  Canvas.Font.Height := -Round(Font.Size * Font.PixelsPerInch {96} / 72);
  s := Memo.Text;
  n := Length(s);
  if n > 2 then
...
{$ENDIF}
    ...
    else if StreamMode = smPrinting then
    begin
      Read(FCalcHeight, 4);
    end;;
  end;
  ...
  HideZero := Boolean(Flags and $10);
  Flags := Flags and not $10;
  if HideZero and (((Format and $0F000000) div $01000000) <> 1) then
    HideZero := False;
end;
...
//    WriteStr(' RTLReading="True"');
  ...
  if (Adjust and frtaRight) <> 0 then
    WriteStr(' HAlign="haRight"')
  else if (Adjust and frtaCenter) <> 0 then
    WriteStr(' HAlign="haCenter"');
    ...
    if ShowModal = mrOk then
      ...
      begin
        ...
        begin
          ...
          (t as TfrMemoView).HideZero := HideZero;
        end;
      end;
  end;
  frFmtForm.Free;
...
{----------------------------------------------------------------------------}
procedure TfrBand.SetBandColumns(Value : integer);
begin
  if (Value < 1) or ((Flags and (flStretched or flBandPageBreak)) <> 0) then
    Value := 1
...
procedure TfrBand.SetBandCurColumn(Value : integer);
begin
  if Value > FBandColumns then
    Value := 1;
  FBandCurColumn := Value;
  if FBandCurColumn = 1 then
    FBandMaxY := Parent.CurY;
  if not (typ in [btCrossHeader, btCrossData, btCrossFooter]) then
    x := (FBandCurColumn - 1) * (Parent.ColWidth + Parent.ColGap) div FBandColumns;
end;
...
begin
  ...
  try
    ...
    begin
      ox := t.x; Inc(t.x, Parent.XAdjust - Parent.LeftMargin + x);
      oy := t.y; Inc(t.y, y);
...
begin
  ...
  begin
    ...
    Page.XAdjust := Parent.XAdjust + t.x + x;
    Page.ColCount := 1;
    Page.PlayFrom := 0;
    Page.OldColWidth := Page.ColWidth;
    Page.ColWidth := Page.ColWidth div BandColumns;
...
begin
  ...
  begin
    ...
    Page := CurReport.Pages[(t as TfrSubReportView).SubPage];
    Page.ClearRecList;
    Page.ColWidth := Page.OldColWidth;
  end;
  Parent.CurY := MaxY;
  Parent.LastBand := nil;
end;
        ...
        if (t.y >= 0) and (t.y < dy) then
          ...
          if (t.y < 0) and (t.y + t.dy >= 0) then
            ...
            begin
              ...
              begin
                t.dy := TfrStretcheable(t).RemainHeight {t.dy} + t.gapy * 2 + 1;
...
begin
  ...
  IsMultiColumnBand := (BandColumns > 1) and (Typ in [btMasterData, btDetailData, btSubDetailData]);
...
// new page was requested in script
  ...
  if (ForceNewPage or ForceNewColumn) and IsMultiColumnBand then
    BandCurColumn := 1;
  ...
  begin
    ...
    begin
      ...
      begin
        if BandCurColumn > 1 then
          Parent.CurY := FBandMaxY
        else //if FBandMaxY = 0 then
          FBandMaxY := Parent.CurY;  
      end;
        ...
        if BandCurColumn = 1 then
          ...
          begin
            ...
            if Parent.Skip then
              ...
              if IsMultiColumnBand and (BandCurColumn = 1) then
                FBandMaxY := Parent.CurY;
            end;
          end;
      EOFReached := True;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        bt.Parent := Bnd;
        Bnd.BandColumns := (Bnd.Flags shr 8) and $F;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Inc(b.Positions[psGlobal]);
            Inc(b.Positions[psLocal]);
            if not b.DataSet.Eof and b.NewPageAfter then begin
              NewPage;
              b.BandCurColumn := 1;
            end;
          end;
...
begin
  SubValue := '';
  if not Assigned(CurView) then Exit;
        ...
        begin
          ...
          begin
            ...
            while (i <= Length(FormatStr)) and (FormatStr[i] in ['0'..'9']) do
              Inc(i);
            Format := Format + 256 * (i-j{StrToInt(Copy(FormatStr, j, i - j)});
          end;
          break;
        end;
        Dec(i);
  ...
  if (CurView is TfrMemoView) and
     TfrMemoView(CurView).HideZero
  then begin
    if ((Format div $01000000) and $0F) = 1 then
    begin
      ZeroStr := FormatValue(0, Format, FormatStr);
      if ParValue = ZeroStr then
        ParValue := '';
    end;
  end;  
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        v := CurValue
      else if s = 'FINALPASS' then
        v := CurReport.FinalPass
...
begin
  ...
  if Title <> '' then
    Printer.Title := 'FreeReport: ' + Title else
    Printer.Title := 'FreeReport: ' + LoadStr(SUntitled);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (dk = dkCount)   and
           Assigned(Field)  and
           (Field.IsNull or
            (VarToStr(Field.Value) = VarToStr(OldV)))
        then
          count := count - 1;

        if (dk = dkCount) and Assigned(Field) then
          OldV := Field.Value;
        DataSet.Next;
      end;
      DataSet.Bookmark := BM;
    ...
    begin
      ...
      if dk <> dkCount then
        s2 := Trim(p3) else
        s2 := Trim(p2);
      if Assigned(AggrBand) and (AggrBand.Typ in [btPageFooter, btMasterFooter, btDetailFooter,
      ...
      begin
        ...
        if not AggrBand.Visible and (CurBand.View <> nil) and (AnsiCompareText(CurBand.View.Name, s1) = 0) then
        begin
