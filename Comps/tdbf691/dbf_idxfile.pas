...
//    property Count: Integer read GetIndexCount;
    ...
    property CurrentParser: TDbfIndexParser read FCurrentParser;
  end;
...
begin
  ...
  case SearchType of
    ...
    stGreater:
      begin
        // while only part of the key we must step forward
        if (findres <> 0) and TDbfFile(FDbfFile).MasterPartialKey then
        begin
          if MatchKey(Key) = 0 then
            findres := 0;
        end;
        if findres = 0 then
        begin
...
begin
  if NeedLocks and (not TDbfFile(FDbfFile).NotIndexResync) then
  begin
...
begin
  if not TDbfFile(FDbfFile).MasterPartialKey then
    Result := DbfCompareString(FCollation, Key1, KeyLen, Key2, KeyLen)
  else
    Result := DbfCompareStringLeft(FCollation, Key1, 0, Key2, KeyLen);
  if Result > 0 then
    Dec(Result, 2);
end;
