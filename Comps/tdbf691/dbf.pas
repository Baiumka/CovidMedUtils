...
{$endif}
    ...
    FOnIndexPack: TDbfPackIndexEvent;
    procedure SetOnIndexPack(Value : TDbfPackIndexEvent);
  ...
  protected
    { abstract methods }
    FLocateParser : TDbfParser;
    FLocateIndex : String;
    FExtraTag : integer;

    FDefaultDbfFields : TDbfFieldDefs;
...
{$endif}
    ...
    procedure SetMasterPartialKey(const Value:Boolean);
    function  GetMasterPartialKey : Boolean;
    procedure SetIndexResync(const Value:Boolean);
    function  GetIndexResync : Boolean;
  public
...
{$endif}
    ...
    procedure CopyFrom(DataSet: TDataSet; FileName: string; DateTimeAsString: Boolean; Level: Integer); overload;
    procedure CopyFrom(DataSet: TDataSet; DateTimeAsString: Boolean; ByName : boolean = False); overload;
    procedure CopyFrom(FileName: string; DateTimeAsString: Boolean); overload;
...
{$endif}
    ...
    property MasterPartialKey : Boolean read GetMasterPartialKey write SetMasterPartialKey;
    property UseIndexResync : Boolean read GetIndexResync write SetIndexResync;
  published
    ...
    property ExtraTag : integer read FExtraTag write FExtraTag;
    ...
    property OnIndexPack: TDbfPackIndexEvent read FOnIndexPack write SetOnIndexPack;
...
var
  I: Integer;
begin
  if Assigned(FDefaultDbfFields) then
    FreeAndNil(FDefaultDbfFields);
...
{$endif}
  ...
  if FDbfFile <> nil then
    FreeAndNil(FDbfFile);
  if (FStorage = stoMemory) and
     (FOpenMode = omAutoCreate) and
     (FUserStream <> nil)
  then
...
begin
  ...
  FDbfFile.OnIndexPack := FOnIndexPack;
end;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          ADbfFieldDefs.Assign(FDefaultDbfFields);
        end
        else begin
          ADbfFieldDefs := TDbfFieldDefs.Create(Self);
          ADbfFieldDefs.DbfVersion := TableLevelToDbfVersion(FTableLevel);
...
function TDbf.HaveFile : Boolean;
var f : string;
begin
  Result := False;
  if Storage <> stoFile then Exit;
  f := AbsolutePath+TableName;
...
procedure TDbf.CopyFrom(DataSet: TDataSet; DateTimeAsString: Boolean; ByName : boolean = False);
...
{$endif}
    ...
    begin
      ...
      begin
        lSrcField := DataSet.Fields[I];
        if ByName then
          lDestField := FindField(lSrcField.FieldName)
        else
          lDestField := Fields[i];
        if Assigned(lDestField) and not lSrcField.IsNull then
        begin
          if lSrcField.DataType = ftDateTime then
          begin
            if FCopyDateTimeAsString then
...
procedure TDbf.CopyFrom(FileName: string; DateTimeAsString: Boolean);
var Temp : TDbf;
begin
  Temp := TDbf.Create(nil);
  try
    Temp.TableName := FileName;
    if Temp.HaveFile then
    begin
      if not Active then
        CopyFrom(Temp, '', DateTimeAsString, 0)
      else
        CopyFrom(Temp, DateTimeAsString, True);
    end;
    Temp.Close;
  finally
    Temp.Free;
...
begin
  ...
  begin
    ...
    // Check for right locate result by Index
    if Assigned(FLocateParser) and (VarArrayDimCount(KeyValues) = 0)
       and VarIsStr(KeyValues)
    then begin
      str := FLocateParser.ExtractFromBuffer(GetCurrentBuffer);
      n := FLocateParser.ResultLen;
      if n <> -1 then
        SetLength(str,n);

      if FDbfFile.UseCodePage = GetOEMCP then
        Translate(@str[1], @str[1], False);
      fstr := KeyValues;
      r := False;
      if Options = [] then
        r := TrimRight(str) <> TrimRight(fstr)
      else if Options = [loCaseInsensitive] then
        r := UpperCase(Trim(str)) <> UpperCase(fstr)
      else if Options = [loPartialKey] then
      begin
        r := (Pos(fstr, str) <> 1);
        if r then
          Result := False;
      end
      else if Options = [loCaseInsensitive, loPartialKey] then
      begin
        r := (Pos(UpperCase(fstr), UpperCase(str)) <> 1);
        if r then
          Result := False;
      end;

      if Result and r then
        raise Exception.CreateFmt('Wrong locate! Index "%s" must be rebuilding! (%s - %s)',
                                  [FLocateIndex, FTableName, Name]);
    end;
  end else
    FCursor.SequentialRecNo := saveRecNo;
end;
...
begin
  ...
  begin
    ...
    //==================================================
    // Search by NAME if KeyFields contain the IndexName
    if lIndexDef = nil then
      lIndexDef := FIndexDefs.GetIndexByName(KeyFields);
    //==================================================
    if lIndexDef <> nil then
    begin
  ...
  begin
    ...
    if Result and (lIndexFile <> nil) then
    begin
      FLocateIndex := lIndexFile.IndexName;
      FLocateParser := lIndexFile.CurrentParser;
    end
    else begin
      FLocateIndex := '';
      FLocateParser := nil;
...
begin
  ...
  else if AIndexName <> '' then
    Result := AIndexName
  else begin
    j := Pos('.', TableName);
    if j<1 then
      j := 256;
    Result := Copy(TableName, 1, j-1);
  end
end;
...
begin
  ...
  lIndexFileName := ParseIndexName(AIndexName);
  // try to delete index
  if FDbfFile.DeleteIndex(lIndexFileName) then
  begin
    i := FIndexDefs.GetIndexByName(AIndexName).Index;
    if i > -1 then
      FIndexDefs.Delete(i);
  end;
...
begin
  ...
  begin
    ...
    if not Assigned(FDefaultDbfFields) then
      FDefaultDbfFields := TDbfFieldDefs.Create(Self);
    Result := FDefaultDbfFields;
    //Result := nil;
  end;
end;
...
begin
  if (FIndexFile = nil) or VarIsNull(LowRange) or VarIsNull(HighRange) then
    exit;
  FillMemory(@LowBuf[0], 101, 0);
  FillMemory(@HighBuf[0], 101, 0);
  ...
  if MasterPartialKey and VarIsStr(LowRange) and VarIsStr(HighRange) then
  begin
    n := Length(LowRange);
    LowBuf[n] := #0;
    n := Length(HighRange);
...
procedure TDbf.SetMasterPartialKey(const Value:Boolean);
begin
  if Assigned(MasterSource) then
    raise Exception.Create('MasetrSource already setted');
  if Assigned(DbfFile) then
    DbfFile.MasterPartialKey := Value;
end;

function  TDbf.GetMasterPartialKey : Boolean;
begin
  if Assigned(FDbfFile) then
    Result := FDbfFile.MasterPartialKey
  else
    Result := False;
end;

procedure TDbf.SetIndexResync(const Value:Boolean);
begin
  if Assigned(FDbfFile) then
    DbfFile.NotIndexResync := not Value;
end;

function  TDbf.GetIndexResync : Boolean;
begin
  if Assigned(FDbfFile) then
    Result := not FDbfFile.NotIndexResync
  else
    Result := True;
end;

procedure TDbf.SetOnIndexPack(Value : TDbfPackIndexEvent);
begin
  FOnIndexPack := Value;
  if Assigned(FDbfFile) then
    FDbfFile.OnIndexPack := Value;
end;
