...
//====================================================================
  ...
  TDbfPackIndexEvent = procedure(CurRecord, TotalRecord : Integer) of object;
...
//====================================================================
  ...
  public
    ...
    property OnIndexPack: TDbfPackIndexEvent read FOnIndexPack write FOnIndexPack;
  end;
...
begin
  ...
  MasterPartialKey := False;
  NotIndexResync := False;
...
begin
  ...
  begin
    ...
    begin
      ...
      if ((FDbfVersion <> xFoxPro) and (PDbfHdr(Header)^.MDXFlag <> 0)) or
          (FDbfVersion = xBaseIII)
      then begin
...
begin
  ...
  DestDbfFile.OnIndexPack  := FOnIndexPack;
...
{$endif}
  try
    try
      while cur <= last do
      begin
        if Assigned(OnIndexPack) then
          OnIndexPack(cur, last);
...
begin
  ...
  FDefaultOpenCodePage := GetOEMCP; //GetACP;
  // the following sets FDefaultCreateLangId
  DefaultCreateCodePage := GetOEMCP; //GetACP;
  FCurrencyAsBCD := true;
