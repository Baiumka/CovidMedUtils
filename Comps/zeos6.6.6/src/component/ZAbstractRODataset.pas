...
{$ENDIF}
  ...
  public
    ...
    function FastLocate(const KeyField: string; const KeyValue: Variant;
...
begin
  ...
  if Index < 0 then
  begin
    if GetBookmarkFlag(Bookmark) = bfInserted then
    begin
      CurrentRow := FInsertIndex;
      Exit;
    end
    else
...
begin
  if TBookmarkFlag(PZRowBuffer(Buffer)^.BookmarkFlag) <> bfInserted then
...
begin
  ...
  if Value = bfInserted then
    FInsertIndex := PZRowBuffer(Buffer)^.Index;
end;
...
function TZAbstractRODataset.FastLocate(const KeyField: string;
  const KeyValue: Variant; Options: TLocateOptions): Boolean;

  function InternalFastLocate(const KeyFields: string;
    const KeyValues: Variant; Options: TLocateOptions): LongInt;
  var
    I, RowNo, rMin, rMax, rComp: Integer;
    FieldRefs: TObjectDynArray;
    FieldIndices: TIntegerDynArray;
    OnlyDataFields: Boolean;
    DecodedKeyValues: TZVariantDynArray;
  ...
  begin
    Result := -1;
    if SortedFields = '' then Exit;
    i := Pos(KeyFields, SortedFields);
    if (i <> 1) then Exit;
    ...
    FieldRefs := DefineFields(Self, KeyFields, OnlyDataFields);
    if not OnlyDataFields or not Assigned(FieldRefs) then Exit;
    FieldIndices := nil;
    DecodedKeyValues := DecodeVariantArray(KeyValues);
    ...
    begin
      ...
      RowNo := Integer(CurrentRows[i]);
      ResultSet.MoveAbsolute(RowNo);
      rComp := CompareFieldsFromResultSetInt(FieldRefs, DecodedKeyValues,
        ResultSet, PartialKey, CaseInsensitive);
      if rComp = 0 then
      begin
        Result := i + 1;
...
begin
  DoBeforeScroll;
  Index := InternalFastLocate(KeyField, KeyValue, Options);
  if Index > 0 then
  begin
    MoveRecNo(Index);
    DoAfterScroll;
