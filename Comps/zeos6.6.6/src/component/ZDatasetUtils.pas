...
{**
  ...
  @param PartialKey <code>True</code> if values should be started with the keys.
  @param CaseInsensitive <code>True</code> if keys are case insensitive.
  @return 0 if values are equal. -1 if values are less. 1 if values are greater.
}
function CompareFieldsFromResultSetInt(const FieldRefs: TObjectDynArray;
...
begin
  ...
  begin
    ...
    begin
      Result := Integer(not ResultSet.IsNull(ColumnIndex));
    ...
    begin
      ...
      Result := AnsiStrLComp(PChar(Value1), PChar(Value2), Length(Value1));
    end
    else
    begin
      case CurrentType of
        stBoolean:
          begin
            Result := Integer(KeyValues[I].VBoolean) -
                      Integer(ResultSet.GetBoolean(ColumnIndex));
          end;
        stByte,
        stShort,
        stInteger,
        stLong:
          begin
            ITemp  := ResultSet.GetLong(ColumnIndex);
            ITemp := KeyValues[I].VInteger - ITemp;
            if ITemp = 0 then
              Result := 0
        ...
        stBigDecimal:
          begin
            DTemp := KeyValues[I].VFloat - ResultSet.GetBigDecimal(ColumnIndex);
            if abs(DTemp) < FLOAT_COMPARE_PRECISION then
              Result := 0
            else if DTemp < 0 then
              Result := -1
            else
               Result := 1;
          end;
        ...
        stTimestamp:
          begin
            DTemp := KeyValues[I].VDateTime - ResultSet.GetTimestamp(ColumnIndex);
            if abs(DTemp) < FLOAT_COMPARE_PRECISION then
              Result := 0
            else if DTemp < 0 then
              Result := -1
        ...
        stUnicodeString:
          begin
            ...
            if CaseInsensitive then
              {$IFNDEF VER130BELOW}
                Value2 := WideUpperCase(ResultSet.GetUnicodeString(ColumnIndex))
              {$ELSE}
                Value2 := AnsiUpperCase(ResultSet.GetUnicodeString(ColumnIndex))
              {$ENDIF}
            else
              Value2 := ResultSet.GetUnicodeString(ColumnIndex);

            Result := AnsiStrComp(PChar(Value1), PChar(Value2));
          end;
      else
          begin
            Value1 := KeyValues[I].VString;
            if CaseInsensitive then
              Value2 := AnsiUpperCase(ResultSet.GetString(ColumnIndex))
            else
              Value2 := ResultSet.GetUnicodeString(ColumnIndex);
            Result := AnsiStrComp(PChar(Value1), PChar(Value2));
          end;
