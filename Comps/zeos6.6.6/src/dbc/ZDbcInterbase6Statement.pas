...
//        raise EZSQLException.Create(SStatementIsNotAllowed);
      ...
      begin
        case InParamValues[i].VType of
          vtNull :
            s := s + Format('Param %d: null'#13#10,[i+1]);
          vtInteger :
            s := s + Format('Param %d: %d'#13#10,[i+1, InParamValues[i].VInteger]);
          vtBoolean :
            s := s + Format('Param %d: %s'#13#10,[i+1, BoolToStr(InParamValues[i].VBoolean, True)]);
          vtFloat :
            s := s + Format('Param %d: %f'#13#10,[i+1, InParamValues[i].VFloat]);
          vtString :
            s := s + Format('Param %d: %s'#13#10,[i+1, InParamValues[i].VString]);
          vtUnicodeString :
            s := s + Format('Param %d: %s'#13#10,[i+1, InParamValues[i].VUnicodeString]);
          vtDateTime :
            s := s + Format('Param %d: %s'#13#10,[i+1, DateTimeToStr(InParamValues[i].VDateTime)]);
        end;
      end;
      DriverManager.LogMessage(lcExecute, GetPlainDriver.GetProtocol, s);
    except
      on E: Exception do begin
       FreeStatement(GetPlainDriver, StmtHandle);
       raise;
...
//        raise EZSQLException.Create(SStatementIsNotAllowed);
      ...
      begin
        ...
        GetPlainDriver.isc_dsql_execute2(@FStatusVector, GetTrHandle, @StmtHandle,
          GetDialect, FParamSQLData.GetData, nil);
        CheckInterbase6Error(SQL);
        GetPlainDriver.isc_dsql_fetch(@FStatusVector, @StmtHandle, GetDialect, SQLData.GetData);
        CheckInterbase6Error(SQL);
        //zRS := TZInterbase6ResultSet.Create(Self, SQL, StmtHandle, '', SQLData, nil, FCachedBlob);
        //zRS.First;
        Result := 1;
        //zRS := nil;
      end
