...
{ TRxParaAttributes }
  ...
  TRxNumberingStyle = (nsParenthesis, nsEnclosed, nsPeriod, nsSimple);
...
begin
  if RichEditVersion < 2 then
    ...
    GetAttributes(Paragraph);
    Result := TRxNumberingStyle(Paragraph.wNumberingStyle div $100);
  end;
end;
...
begin
  ...
  if (L >= 2) and (Text[L - 2] = #13) and (Text[L - 1] = #10) then Dec(L, 2)
  else if (RichEditVersion >= 2) and (L >= 1) and (Text[L - 1] = #13) then Dec(L);
