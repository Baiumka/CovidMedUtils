...
begin
  ...
  begin
    ...
    TempBM.Canvas.Font.Assign(Font);
    ACanvas := TempBM.Canvas;
  end;
  try
    if (Win32Platform <> VER_PLATFORM_WIN32_NT) or (ACanvas.Font.Charset <> DEFAULT_CHARSET) then
    begin
      Ansis := Text;
      GetTextExtentExPoint(ACanvas.Handle, PAnsiChar(Ansis), Length(Ansis) , Round(Width), @Cnt, nil, Sz);
    end
    else
      GetTextExtentExPointW(ACanvas.Handle, PwideChar(Text), Length(Text) , Round(Width), @Cnt, nil, Sz);
  finally
    if Assigned(TempBM) then
      TempBM.Free;
  end;}
  NCnt  := Round(Width/fr1CharX);
  Ansis := Text;
  Cnt   := Length(Ansis);
  if Cnt > NCnt then
    Result := Copy(Text, 1, NCnt)
  else
    Result := Text;
