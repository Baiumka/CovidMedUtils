...
var
  ...
  function FormatData(const Value: Variant;
  ...
  begin
    ...
    try
      TValue := Report.Calc(ExprStr);
      Result := FormatData(TValue, Format);
    finally
      if Format <> nil then
        Format.Free;
    end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        s2 := CalcAndFormat(s1); //VarToStr(Report.Calc(s1));
...
//    FRichEdit.SelLength := 1;
    FRichEdit.Paragraph.FirstIndent := FRichEdit.Paragraph.FirstIndent + FRichEdit.Paragraph.LeftIndent;
    FRichEdit.Paragraph.LeftIndent := 0;
    
    FRichEdit.Paragraph.Numbering      := nsNone;
    FRichEdit.Paragraph.NumberingStyle := nsSimple;
  end;
  ...
  begin
    ...
    if (FSaveHeight > Height) and (Result < 0) and (LastChar < FRichEdit.GetTextLen) then
      Result := 0; 
    if not UsePrinterCanvas then
      ReleaseDC(0, hdc);
  end;
  ...
  try
    ...
    FRichEdit.SelLength := FRichEdit.GetTextLen - FRichEdit.SelStart + 1;
