...
{$ENDIF}
    ...
    FOnBeforePreviewDialog : TNotifyEvent;
...
{$ENDIF}
  ...
  published
    ...
    property OnBeforePreviewDialog: TNotifyEvent read FOnBeforePreviewDialog write FOnBeforePreviewDialog;
  end;
...
begin
  ...
  try
    ...
    if FHideZeros and (TVarData(FValue).VType <> varString) and
      (TVarData(FValue).VType <> varOleStr) and ((FValue = null) or (FValue = 0))
    then
      Result := ''
    else begin
      ...
      if FHideZeros then
      begin
        FormatStr := FormatData(0.00, Format);
        if FormatStr = Result then
          Result := '';
      end;
    end;
  finally
  ...
  try
    case AFormat.Kind of
      ...
      fkDateTime: begin
        ...
        else if StrToFloatDef(Value, 0.00) > 0.00 then
          Result := FormatDateTime(AFormat.FormatStr, Value)
        else
          Result := Value;
      end;
