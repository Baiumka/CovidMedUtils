  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (b is TfrxReportTitle) or (b is TfrxReportSummary) or
           (b is TfrxPageHeader) or (b is TfrxPageFooter)
        then
          VC := False
        else if b is TfrxChild then
        begin
          t := nil;
          t1 := b;
          while True do
          begin
            ...
            begin
              t := FPage.Objects[j];
              if (t is TfrxBand) and not TfrxBand(t).Vertical then
                if TfrxBand(t).Child = t1 then
                  Break;
              t := nil;    
            end;
            if (t is TfrxReportTitle) or (t is TfrxReportSummary) or
               (t is TfrxPageHeader) or (t is TfrxPageFooter)
            then begin
              VC := False;
              Break;
            end
            else if t is TfrxChild then
            begin
              t1 := t;
            end
            else break;
...
begin
  ...
  begin
    SaveCurX := CurX;
    with TfrxReportPage(Band.Page) do
      if IsSubReport then
        if SubReport.PrintOnParent then
         if CurX <> SubReport.OriginLeft then
            CurX := SubReport.OriginLeft;
...
begin
  ...
  for i := 0 to Band.Objects.Count - 1 do
    ...
    begin
      ...
      CurX := SaveCurX + Sub.Left;
      CurY := SaveCurY - Band.FStretchedHeight{Height} + Sub.Top; //SaveCurY - Sub.Height;
      { ...and run the subreport }
      RunPage(Sub.Page);
