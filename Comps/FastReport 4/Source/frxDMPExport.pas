    ...
    begin
      for i := 1 to Length(AnsiStr) do
        if AnsiStr[i] = #160 then
          AnsiStr[i] := #32
        else if AnsiStr[i] = #178 then
          AnsiStr[i] := #73
        else if AnsiStr[i] = #179 then
          AnsiStr[i] := #105;
...
var
  Style: Integer;
  Memo1: TfrxDMPMemoView;
begin
  if (Obj is TfrxView) and not TfrxView(Obj).Printable then Exit;
  if Obj is TfrxDMPMemoView then
  begin
    Memo1 := TfrxDMPMemoView(Obj);
    Style := StyleToInt(Memo1.FontStyle);
    DrawMemo(Round(Memo1.AbsLeft / fr1CharX), Round(Memo1.AbsTop / fr1CharY),
      Round(Memo1.Width / fr1CharX), Round(Memo1.Height / fr1CharY), Memo1);
    if (ftLeft in Memo1.Frame.Typ) then
      DrawFrame(Round(Memo1.AbsLeft / fr1CharX) - 1,
        Round(Memo1.AbsTop / fr1CharY) - 1, 1, Round(Memo1.Height / fr1CharY) + 2, Style);
    if (ftRight in Memo1.Frame.Typ) then
      DrawFrame(Round((Memo1.AbsLeft + Memo1.Width) / fr1CharX),
        Round(Memo1.AbsTop / fr1CharY) - 1, 1, Round(Memo1.Height / fr1CharY) + 2, Style);
    if (ftTop in Memo1.Frame.Typ) then
      DrawFrame(Round(Memo1.AbsLeft / fr1CharX) - 1,
        Round(Memo1.AbsTop / fr1CharY) - 1, Round(Memo1.Width / fr1CharX) + 2, 1, Style);
    if (ftBottom in Memo1.Frame.Typ) then
      DrawFrame(Round(Memo1.AbsLeft / fr1CharX) - 1,
        Round((Memo1.AbsTop + Memo1.Height) / fr1CharY),
        Round(Memo1.Width / fr1CharX) + 2, 1, Style);
  end
