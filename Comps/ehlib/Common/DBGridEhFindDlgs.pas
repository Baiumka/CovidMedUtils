...
function CorrectTitle(const Value : string) : string;
var j : integer;
begin
  Result := Value;
  for j := 0 to Length(Result) do
    if Result[j] in [#13,#10] then
      Result[j] := ' ';
end;
...
begin
  ...
  FGrid := AGrid;
  FillColumnsList;
  cbFindIn.ItemIndex := cbFindIn.Items.IndexOf(CorrectTitle(FGrid.Columns[FGrid.SelectedIndex].Title.Caption));
  IsFirstTry := True;
  Close;
...
var
  ...
  DataText: String;
  OldBeroreScroll, OldAfterScroll : TDataSetNotifyEvent;
begin
  ...
  begin
    ...
    begin
      ...
      OldBeroreScroll := Grid.DataSource.DataSet.BeforeScroll;
      OldAfterScroll  := Grid.DataSource.DataSet.AfterScroll;
      ...
      try
        ...
        Grid.DataSource.DataSet.BeforeScroll := OldBeroreScroll;
        Grid.DataSource.DataSet.AfterScroll  := OldAfterScroll;
        Grid.DataSource.DataSet.EnableControls;
        if RecordFounded then
        begin
          TDataSetEh(Grid.DataSource.DataSet).DoBeforeScroll;
          TDataSetEh(Grid.DataSource.DataSet).DoAfterScroll;
        end;
      end;
...
begin
  ...
  begin
    for i := 0 to Length(FColumnFields)-1 do
      cbFindIn.Items.AddObject(CorrectTitle(FColumnFields[i].Caption), FColumnFields[i].Column);
  end else
  begin
    for i:= 0 to FGrid.VisibleColumns.Count-1 do
      cbFindIn.Items.AddObject(CorrectTitle(FGrid.VisibleColumns[i].Title.Caption),
...
var CP : TColCellParamsEh;
begin
  if cbUseFormat.Checked then
  begin
    if Assigned(Col.OnGetCellParams) then
    begin
      CP := TColCellParamsEh.Create;
      Col.FillColCellParams(CP);
      Col.GetColCellParams(False, CP);
      Result := CP.Text;
      FreeAndNil(CP);
    end
    else
