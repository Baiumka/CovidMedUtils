...
{*******************************************************}
{============== Changes by Vadim ========================
#1  05.08.2010 добавил доработки от Александра Сырова
}
{$I EhLib.Inc}
...
procedure ConvertVarStrValues(var v: Variant; ot: TSTOperandTypeEh);
  function StrToDateTime(s :String) :Variant; {#1}
  begin
...
begin
  Result := '';
  if     (AGrid.DataSource <> nil)
     and (AGrid.DataSource.DataSet <> nil)
     //and AGrid.DataSource.DataSet.Active {#1}
  then begin
...
const
  Brackets : array[False..True,0..1] of string[1] =
             (('', ''), ('[', ']'));
function AddBracketField(const AValue : String) : string;
begin
  Result := Brackets[BracketField,0]+AValue+Brackets[BracketField,1];
end;
...
begin
  ...
  begin
    ...
    if VarIsArray(v) then
      for i := VarArrayLowBound(v, 1) to VarArrayHighBound(v, 1) do
        Result := Result + AddBracketField(FieldName)+' = ' + VarValueAsFilterStr(v[i]) + ' OR '
    else
      Result := Result + AddBracketField(FieldName)+' = ' + VarValueAsFilterStr(v) + ' OR ';
    Delete(Result, Length(Result) - 3, 4);
    Result := Result + ')';
  end
  else if O in [foLike, foNotLike] then
  begin
    Result := Result + ' ' + AddBracketField(FieldName);
  ...
  begin
    Result := Result +  ' ' + AddBracketField(FieldName) + ' ' + STFilterOperatorsSQLStrMapEh[O];
...
begin
  ...
  try
    ...
    for i := 0 to SQL.Count - 1 do
      ...
      else if not HaveWhere and (Pos('WHERE ',AnsiUpperCase(SQL[i])) > 0) then
        HaveWhere := True;
    ...
    try
      ...
      if HaveWhere then
        ...
        SQL.Strings[OrderLine] := SQLFilterMarker + 'WHERE (' + s + ')';
