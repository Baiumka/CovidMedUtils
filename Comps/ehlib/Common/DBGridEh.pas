...
{*******************************************************}
{============== Changes by Vadim ========================
#1  30.07.2010 filter the char for numeric field
#2  30.07.2010 filter the char for numeric field
#3  04.08.2010 add to column new property ShowRecNo
#4  17.08.2010 add ShowColumnEditDialog
#5  04.11.2010 add new event TGetCellEhParamsEventEh
#6  procedure UpdateRowCount moved from protected section
#7  Change Draw Footer When StaticText then ShowAlLways
#8  add NoColumnMove
}
{$I EhLib.Inc}
...
{    for lookup drop-down grid    }
  ...
  {#5}
  TGetCellEhParamsEventEh = procedure(Sender: TObject; Field: TField;
...
{ TColumnEh }
  ...
  TGetColCellKeyValue = procedure(Sender : TObject; var KeyValue : string) of object;
  TGetManualEditStyle = procedure(Sender : TObject; var KeyValue : integer) of object;
...
//    FOnGetItemImageIndex: TListGetImageIndexEventEh;
    ...
    FShowRecNo : Boolean; {#3}
    FTag: Longint;
    FTitle: TColumnTitleEh;
    FToolTips: Boolean;
    FUpdateData: TColCellUpdateDataEventEh;
    FGetKeyValue : TGetColCellKeyValue;
    FGetManualEditStyle : TGetManualEditStyle;
    FVisible: Boolean;
    FWidth: Integer;
...
{$ENDIF}
    ...
    property ShowRecNo : Boolean read FShowRecNo write SetShowRecNo default False;
...
//    property OnGetItemImageIndex: TListGetImageIndexEventEh read FOnGetItemImageIndex write FOnGetItemImageIndex;
    ...
    property OnGetKeyValue : TGetColCellKeyValue read FGetKeyValue write FGetKeyValue;
    property OnGetManualEditStyle : TGetManualEditStyle read FGetManualEditStyle write FGetManualEditStyle;
  end;
...
//    property OnGetItemImageIndex;
    ...
    property OnGetKeyValue;
    property OnGetManualEditStyle;
  end;
...
{ TCustomDBGridEh }
  ...
  TDBGridEhOption = (dghFixed3D, dghFrozen3D, dghFooter3D, dghData3D, dghResizeWholeRightPart,
    ...
    dghRecordMoving, {#4}dghDialogColumnEdit{#4}, {#8} dghNoColumnMove);
...
(*  TDrawDataCellEvent = procedure (Sender: TObject; const Rect: TRect; Field: TField;
  ...
  TSumListCheckEh = procedure(Column : TColumnEh; FooterNo : Integer; var Handled : Boolean) of object;
...
//    FSortMarkerImages: TImageList;
    ...
    FDisableButtonWhenReadOnly : Boolean;
...
{$ENDIF}
    ...
    {#5}
    procedure GetCellParamsEh(Field: TField; AFont: TFont;
      var Background: TColor; Highlight: Boolean); dynamic;
    {---}
...
{$ENDIF}
    ...
    procedure SumListCheck(Sender : TObject; var Handled : Boolean);
...
{$ENDIF}
    ...
    procedure ShowColumnEditDlg; {#4}
    procedure ShowFilterDlg; {#4}
    //procedure UpdateRowCount; virtual; {#6}
...
{$ENDIF}
    ...
    {#5}
    property OnGetCellParamsEh: TGetCellEhParamsEventEh read FOnGetCellParamsEh write FOnGetCellParamsEh;
    {---}
    ...
    property OnSumListChanged : TNotifyEvent read FOnSumListChanged write FOnSumListChanged;
    property OnSumListRecalcAll: TNotifyEvent read FOnSumListRecalcAll write FOnSumListRecalcAll;
    property OnSumListCheck: TSumListCheckEh read FOnSumListCheck write FOnSumListCheck;
    ...
    property OptionsEh: TDBGridEhOptions read FOptionsEh write SetOptionsEh
      default [dghFixed3D, dghHighlightFocus, dghClearSelection, dghDialogFind, dghDialogColumnEdit];
...
//    property HTitleMargin: Integer read FHTitleMargin write WritEhTitleMargin default 0;
  published
    property DisableButtonWhenReadOnly : Boolean read FDisableButtonWhenReadOnly write FDisableButtonWhenReadOnly default False;
  end;
...
{$ENDIF}
    ...
    property OnSumListChanged;
    property OnSumListCheck;
...
var
  ...
  DBGridEhColumnEditKey : TShortCut; {#4}
...
{$IFDEF EH_LIB_7} Themes, UxTheme, {$ENDIF}
  ...
  DBLookupEh, ColumnEdit, DateUtils;
...
//      FDataList.RowCount := Column.DropDownRows;
        ...
        s := LookupKeyFields;
        if (s = '') and (Column.LookupDisplayFields <> '') then
        begin
          s := Column.LookupDisplayFields;
          I := Pos(';',s);
          if I > 0 then
            SetLength(s,I-1);
        end;
        //FDataList.KeyField := LookupKeyFields;
        FDataList.KeyField := s;
...
begin
  ...
  begin
    ...
    begin
      ...
      ExecuteDBGridEhFindDialogProc(Grid, SelText, '', nil, Grid.IsFindDialogShowAsModal)
    else if (ShortCut(Key, Shift) = DBGridEhColumnEditKey) and (dghDialogColumnEdit in Grid.OptionsEh) then {#4}
    begin
      if     ((dghIncSearch in Grid.OptionsEh) or (dghPreferIncSearch in Grid.OptionsEh))
         and Grid.InplaceSearching
      then Grid.StopInplaceSearch;

      ShowColumnEdit(Grid, nil);
    end
...
begin
    ...
    cbsAuto:
      if Assigned(Column.Field) then
        ...
        { Show the dropdown button only if the field is editable }
          ...
          if Assigned(Column.OnGetManualEditStyle) then
          begin
            EditStyleIndex := Integer(NewStyle);
            Column.OnGetManualEditStyle(Column, EditStyleIndex);
            if EditStyleIndex in [0..8] then
              NewStyle := TEditStyle(EditStyleIndex);
          end;
        end;
  ...
  begin
    ...
    Grid.GetCellParamsEh(Column.Field, FFont, FBackground, True);
...
begin
  if Grid.AllowedOperationUpdate and Column.CanModify(False) then
    ...
    begin
      ...
      inherited;
      if ((EditStyle = esDateCalendar) and (Column.GetColumnType = ctCommon)) then
      begin
        Grid.FEditText := Text;
      end;
    end;
...
var NewEditButtonControlsCount, OldEditButtonControlsCount: Integer;
  ...
  function AllwaysDisable : Boolean;
  begin
    Result := False;
    if Grid.DisableButtonWhenReadOnly then
    begin
      Result := (Grid.ReadOnly or
                 Column.ReadOnly or
                 ((Grid.DataSource <> nil) and
                  (Grid.DataSource.DataSet <> nil) and
                  (Grid.DataSource.DataSet.CanModify)));
      Result := not Result;
    end;
  end;
  ...
  begin
    ...
    ControlRec.EditButtonControl.Enabled := Button.Enabled and not AllwaysDisable;
...
begin
  ...
  begin
    ...
    begin
      ...
      FEditButtonControlList[i].EditButtonControl := nil;
      FEditButtonControlList[i].ButtonLine.Free;
      FEditButtonControlList[i].ButtonLine := nil;
    end;
  ...
  begin
    ...
    begin
      ...
      EditButtonControl.Enabled := not AllwaysDisable;
      case EditStyle of
...
begin
  if Assigned(Field) and (KeyList.Count = 0) and (PickList.Count = 0)
...
{$ENDIF}
    ...
    DateTimeToString(Result, DisplayFormat, Field.AsDateTime)
  else if (Field is TBlobField) and (AnsiUpperCase(DisplayFormat) = 'MEMO') then
...
begin
  ...
  begin
    Result := Field.Text;
    if Assigned(OnGetKeyValue) then
      OnGetKeyValue(Self, Result);
    KeyIndex := KeyList.IndexOf(Result);
...
begin
  if Field <> nil then
    ...
    begin
      Result := cbGrayed;
      if (KeyList.Count > 1) and (NotInKeyListIndex >= 0) then
      begin
        if NotInKeyListIndex = 0 then
          Result := cbChecked
        else
          Result := cbUnchecked;
      end;
...
begin
  if ShowRecNo then
    with ColCellParamsEh do
       Text := IntToStr(Grid.DataSource.DataSet.RecNo);
...
var Processed: Boolean;
  procedure DateValueCheck(MSG : String);
  var Y,M,D : Word;
  begin
    if  Grid.GetEditMask(Grid.Col, Grid.Row) <> '!99/99/9999;1; ' then
      raise Exception.Create(MSG);
    D := StrToIntDef(Trim(Copy(Text,1,2)),0);
    M := StrToIntDef(Trim(Copy(Text,4,2)),0);
    Y := StrToIntDef(Trim(Copy(Text,7,4)),0);
    if D = 0 then
      raise Exception.Create(MSG);
    if (M = 0) then
    begin
    ...
    if Y = 0 then
      Y := YearOf(Date);
    Field.Text := Format('%.2d.%.2d.%.4d',[D,M,Y]);
  end;
begin
  if Grid <> nil then
  begin
    ...
    begin
      if Text = FormatMaskText(Grid.GetEditMask(Grid.Col, Grid.Row), '') then
        Field.Value := null
      else
      try
        Field.Text := Text;
...
procedure TColumnEh.SetShowRecNo(const Value: Boolean);
begin
    ...
    begin
     ...
     ReadOnly   := True;
      ...
      if FieldName = '' then
        ...
        raise Exception.Create('Only when FieldName is empty');
    end
  end;
end;
...
//  FTitleOffset := 0;
  ...
  FSumList.OnProducerDBSum := SumListCheck;
  ...
  FOptionsEh := [dghFixed3D, dghHighlightFocus, dghClearSelection, dghDialogFind,
                 dghDialogColumnEdit, dghNoColumnMove];
...
var //isNumericField, isDateField : Boolean;
    S : String;
begin
    ...
    begin
      ...
      if Assigned(KeyList) and (KeyList.Count > 0)
        then Result := Result
          ...
          {#1 isNumericField isDateField}
          if (Field.DataType in [ftFloat, ftCurrency]) and
             (Key in [',','.'])
          then begin
            Key := DecimalSeparator;
            if not (Key in Field.ValidChars) then
              Field.ValidChars := Field.ValidChars + [Key];
          end
          else if (Field.DataType in [ftDate, ftDateTime]) and
                  (Key in [',','.','-','/'])
          then
            Key := DateSeparator;

          Result := Result and Field.IsValidChar(Key);

          if Result and
             (Field.DataType in [ftSmallint, ftInteger, ftWord, ftFloat, ftCurrency, ftLargeint]) and
             (Key in [DecimalSeparator, '+', '-'])
          then begin
            if Pos(Key, TDBGridInplaceEdit(InplaceEditor).Text) > 0 then
            begin
              if (TDBGridInplaceEdit(InplaceEditor).SelLength = 1) or
                 (Pos(Key, TDBGridInplaceEdit(InplaceEditor).SelText) = 0) then
                Result := False;
            end
            else begin
              if (Key in ['+','-']) then
              begin
                if TDBGridInplaceEdit(InplaceEditor).SelStart > 0 then
                  Result := False
                else begin
                  S := TDBGridInplaceEdit(InplaceEditor).Text;
                  if (S <> '') and
                     (TDBGridInplaceEdit(InplaceEditor).SelLength = 1) and
                     (TDBGridInplaceEdit(InplaceEditor).SelText = '') and
                     (((Key = '+') and (S[1] = '-')) or
                      ((Key = '-') and (S[1] = '+'))) then
                    Result := False;
                end;
...
//      InflateRect(DHRect, 1, 1);
  ...
  var
    OldActive: Integer;
    CheckLeftValue : Boolean;
    LeftValue : string;
    l, j : Integer;
    LeftColumn : TColumnEh;
    ...
    begin
      ...
      begin
        ...
        GetCellParamsEh(Column.Field, FFont, FBackground, Highlight);
      ...
      begin
        Result := True;
        if CheckLeftValue then
        begin
          s1 := '';
          j := l;
          while j > 0 do
          begin
            LeftColumn := TColumnEh(Self.VisibleColumns.Items[j-1]);
            if LeftColumn.HideDuplicates then
              s1 := s1+LeftColumn.DisplayText
            else
              break;
            dec(j);  
          end;
  ...
  begin
    ...
    l := Self.VisibleColumns.IndexOf(Column);
    if l > 0 then
    begin
      LeftValue := '';
      j := l;
      ...
      begin
        LeftColumn := TColumnEh(Self.VisibleColumns.Items[j-1]);
        if LeftColumn.HideDuplicates then
          LeftValue := LeftValue+LeftColumn.DisplayText
        else
          break;
        dec(j);
      end;
...
//\\
        ...
        then begin
               ...
               begin
                 ...
                 begin
                   ...
                   FillRect(ARect);
                   if (Footer.ValueType = fvtStaticText) then
                   begin
                     Value := Footer.Value;
                     WriteCellText(DrawColumn, Canvas, ARect, IsFillTextCell,
                                   XFrameOffs, YFrameOffs, Value, Footer.Alignment,
                                   DrawColumn.Layout, Footer.WordWrap and FAllowWordWrap,
                                   Footer.EndEllipsis, 0, 0);
                   end;
                 end;
               end
               else {-----------------}
      ...
      begin // Draw contents
        ...
        begin
          ...
          {#5}
          GetCellParamsEh(DrawColumn.Field, Font, FColCellParamsEh.FBackground, HighLight);
...
begin
  ...
  if FDatalink.Active then
    with Columns[RawToDataColumn(ACol)] do
      if EditMask <> '' then
        Result := EditMask
      else if Assigned(Field) and (Field.DataType = ftDate) then
        Result := '!99/99/9999;1; '
    ...
    with Col do
      ...
      begin
        ...
        begin
          ...
          if Assigned(Col.OnGetManualEditStyle) then
          begin
            KeyIndex := Integer(esSimple);
            Col.OnGetManualEditStyle(Col, KeyIndex);
            if KeyIndex = Integer(esDataList) then
              FEditKeyValue := Field.Text
            else
              FEditKeyValue := Null;
          end
          else
...
var
  ...
  begin
    if (FBookmarks.Count > 1)
      then Msg := SDeleteMultipleRecordsQuestionEh
      else Msg := SDeleteRecordQuestionEh;
...
begin
  ...
  if (ShortCut(Key, Shift) = DBGridEhSetValueFromPrevRecordKey) then
    ...
    ExecuteDBGridEhFindDialogProc(Self, '', '', nil, IsFindDialogShowAsModal)
  else if (ShortCut(Key, Shift) = DBGridEhColumnEditKey) and (dghDialogColumnEdit in OptionsEh) then {#4}
  begin
    if     ((dghIncSearch in OptionsEh) or (dghPreferIncSearch in OptionsEh))
       and InplaceSearching
    then StopInplaceSearch;

    ShowColumnEdit(Self, nil);
  end
  else if FInplaceSearching then
...
begin
  ...
  with Columns[SelectedIndex] do
    if FDatalink.Active and Assigned(Field) then
    begin
      isNumericField := (Field.DataType in [ftSmallint, ftInteger, ftWord,
                                            ftFloat, ftCurrency, ftLargeint]);
      isDateField    := (Field.DataType in [ftDate, ftDateTime]);
      if isNumericField and
         (Field.DataType in [ftFloat, ftCurrency]) and
         not (DecimalSeparator in Field.ValidChars)
      then
        Field.ValidChars := Field.ValidChars + [DecimalSeparator];
    end
    else begin
      isNumericField := False;
      isDateField    := False;
    end;
  if isNumericField and (Key in [',','.']) then
    Key := DecimalSeparator
  else if isDateField and (Key in [',','.','-','/']) then
    Key := DateSeparator;
  ...
  begin
    if FInplaceSearching
      then StartInplaceSearch(Key{GetCompleteKeyPress}, FInplaceSearchTimeOut, inpsFromFirstEh)
      else StartInplaceSearch(Key{GetCompleteKeyPress}, DBGridEhInplaceSearchTimeOut, inpsFromFirstEh);
    Key := #0;
  end;
end;
...
begin
  ...
  begin
    ...
    if not ((AGridState = gsColSizing) and (dghDblClickOptimizeColWidth in OptionsEh)) and
       (Cell.Y > 0)
    then
      DblClick;
  ...
  begin
    ...
    begin
      ...
      if (FDataLink <> nil) and (AComponent = DataSource) then
        DataSource := nil;
      if (SumList <> nil) and (DataSource = nil) and SumList.Active  then
...
//        NewGridOptions := NewGridOptions + [goFixedVertLine];
    ...
    begin
      ...
      {#8}
      if not (dghNoColumnMove in FOptionsEh) then
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          //RowHeights[TopDataOffset + 0] := RowHeights[0];
        end;
      end;
    end else
    begin
      ...
      begin
        ...
        //RowHeights[TopDataOffset + VisibleDataRowCount] := RowHeights[0];
      end else
        SetRowCount(NewRowCount);
    ...
    if FDatalink.Active then
      with FDatalink.Dataset do
        if (dgCancelOnExit in Options) and (State = dsInsert) and
          {not Modified and} not FDatalink.FModified then {update temp}
          Cancel else
          FDataLink.UpdateData;
  except
    SetFocus;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        FRow := 0;
        GetCellParams(Column, FFont, FBackground, FState);
        GetCellParamsEh(Column.Field, FFont, FBackground, False);
...
procedure TCustomDBGridEh.GetCellParamsEh(Field: TField; AFont: TFont;
  var Background: TColor; Highlight: Boolean);
begin
  if Assigned(FOnGetCellParamsEh) then
    FOnGetCellParamsEh(Self, Field, AFont, Background, Highlight);
end;
...
begin
  InvalidateFooter;
  if Assigned(FOnSumListChanged) then
    FOnSumListChanged(Sender);
end;
  ...
  case Footer.ValueType of
    ...
    fvtSum, fvtAvg:
      begin
        ...
        with Field do
        begin
          v := SumList.SumCollection.GetSumByOpAndFName(SumListArray[Footer.ValueType], FieldName,
                Footer.FColumn, Footer.Index +1).SumValue;
          case DataType of
...
{              ftDate, ftTime, ftDateTime:
    ...
    fvtCount:
      if Footer.DisplayFormat <> '' then
        Result := FormatFloat(Footer.DisplayFormat, SumList.SumCollection.GetSumByOpAndFName(goCount, Footer.FieldName,
                  Footer.FColumn, Footer.Index +1).SumValue)
      else
        Result := FloatToStr(SumList.SumCollection.GetSumByOpAndFName(goCount, Footer.FieldName,
                Footer.FColumn, Footer.Index +1).SumValue);
    fvtFieldValue:
...
procedure TCustomDBGridEh.SumListCheck(Sender : TObject; var Handled : Boolean);
begin
  if Assigned(FonSumListCheck) and Assigned(Sender) and
    (Sender is TDBSum)
  then begin
    FOnSumListCheck(TColumnEh(TDBSum(Sender).Column), TDBSum(Sender).FooterNo, Handled); 
  end;
end;
...
begin
  ...
  begin
    NewGridOptions := inherited Options;
    if dghNoColumnMove in Value then
      NewGridOptions := NewGridOptions - [goColMoving]
    else
      NewGridOptions := NewGridOptions + [goColMoving];
    inherited Options := NewGridOptions;
...
{$ENDIF}
  ...
  begin
    ...
    begin
      ...
      begin
        s := Column.Field.Text;
        if Assigned(Column.OnGetKeyValue) then
          Column.OnGetKeyValue(Self, s);
        KeyIndex := Column.KeyList.IndexOf(s);
      end
      else KeyIndex := (Integer(Column.CheckboxState) + 1) mod 2;
      if KeyIndex = -1 then
        KeyIndex := Column.NotInKeyListIndex;
...
{$ENDIF}
          ...
          begin
            ...
            FRow := Cell.Y;
            GetCellParams(Column, FFont, FBackground, FState);
            GetCellParamsEh(Column.Field, FFont, FBackground, False); {#5}
            Column.GetColCellParams(False, FColCellParamsEh);
            s := FText;
...
procedure TCustomDBGridEh.WMLButtonDblClk(var Message: TWMLButtonDblClk);
{var
  Coord: TGridCoord;}
begin
  {Coord := MouseCoord(Message.XPos, Message.YPos);
  if (Coord.Y > 0) or not (dgTitles in Options) then}
    inherited;
end;
...
var OldAfterScroll, OldBeforeScroll : TDataSetNotifyEvent;
begin
  if not DataLink.Active then Exit;
      ...
      begin
        ...
        try
          ...
          try
            ...
            begin
              TDataSetEh(Self.DataSource.DataSet).DoBeforeScroll;
              TDataSetEh(Self.DataSource.DataSet).DoAfterScroll;
            end;
          finally
...
procedure TCustomDBGridEh.ShowColumnEditDlg;
begin
  if (dghDialogColumnEdit in OptionsEh) then {#4}
    ShowColumnEdit(Self, nil)
end;

procedure TCustomDBGridEh.ShowFilterDlg;
begin
  //if (dghDialogColumnEdit in OptionsEh) then {#4}
  //  ShowColumnEdit(Self, nil)
  if InplaceSearching then
    StopInplaceSearch;
  ExecuteDBGridEhFindDialogProc(Self, '', '', nil, IsFindDialogShowAsModal);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FDBSum.Column := TObject(FColumn);
      FDBSum.FooterNo := 0;
      if Self.Collection <> nil then
        FDBSum.FooterNo := Self.Index+1;
      case ValueType of
        fvtSum, fvtAvg:
          begin
            if ValueType = fvtSum
...
{$IFDEF CIL}
  Sort(CompareColums);
{$ELSE}
  Sort(TListSortCompare(@CompareColums));
{$ENDIF}
end;
...
initialization
  ...
  DBGridEhColumnEditKey := ShortCut(Word('G'), [ssCtrl]); {#4}
