...
{*******************************************************}
{============== Changes by Vadim ========================
#1  20.07.2010 numeric format
#2  20.07.2010 numeric format
#3  04.08.2010 change in TCustomDBEditEh.Destroy
#4  04.08.2010
#5  04.08.2010 ƒобавлены доработки от јлександра —альникова
#6  04.08.2010 TEditButtonEh.Enabled
#7  09.11.2010 add property DisableButtonWhenReadOnly
#8  22.11.2010 add property UserDataChange
}
{$I EhLib.Inc}
...
{ TCustomDBEditEh }
  ...
  {#8}
  TUserDataChangeEventEh = procedure(Sender : TObject; AValue: Variant) of object;
  ...
  private
    ...
    FOnUserDataChange : TUserDataChangeEventEh;
    FReadOnly: Boolean;
    FShowHint: Boolean;
    FTooltips: Boolean;
...
{$ENDIF}
    ...
    procedure SetDisableButtonWhenReadOnly(const Value: Boolean);
  protected
    ...
    FDisableButtonWhenReadOnly : Boolean; {#7}
...
{$ENDIF}
  ...
  public
    ...
    {#8}
    property OnUserDataChange : TUserDataChangeEventEh read FOnUserDataChange write FOnUserDataChange;
  {#7}
  published
    property DisableButtonWhenReadOnly : Boolean read FDisableButtonWhenReadOnly write SetDisableButtonWhenReadOnly default False;
  {----}
  end;
...
{$IFDEF EH_LIB_6}
    FExtItemsEh : TStringListEh;
{$ENDIF}
    FOnGetItemsListEh: TGetItemsListEh;
    function GetImages: TCustomImageList;
    procedure CMCancelMode(var Message: TCMCancelMode); message CM_CANCELMODE;
    procedure CMMouseWheel(var Message: TCMMouseWheel); message CM_MOUSEWHEEL;
    procedure CMWantSpecialKey(var Message: TCMWantSpecialKey); message CM_WANTSPECIALKEY;
    ...
    procedure WMSetCursor(var Message: TWMSetCursor); message WM_SETCURSOR;
    procedure SetOnGetItemsListEh(const Value : TGetItemsListEh);
  protected
    FItemsCount: Integer;
    FKeyBased: Boolean;
    FVarValue: Variant;
  ...
  public
    ...
    property OnGetItemsListEh: TGetItemsListEh read FOnGetItemsListEh write SetOnGetItemsListEh;
...
{$ENDIF}
    ...
    property OnGetItemsList;
    property OnGetItemsListEh;
    property OnKeyDown;
    property OnKeyPress;
    property OnKeyUp;
    property OnMouseDown;
...
{ TCustomDBCheckBoxEh }
  TOnGetSetCheckValue = procedure(Sender : TObject; var Value : string) of object;
  ...
  private
    ...
    FMouseAboveControl: Boolean;
    FOnUpdateData: TUpdateDataEventEh;
    FOnGetValue : TOnGetSetCheckValue;
    FOnSetValue : TOnGetSetCheckValue;
...
//    procedure Paint; override;
  ...
  public
    ...
    property  OnGetValue: TOnGetSetCheckValue read FOnGetValue write FOnGetValue;
    property  OnSetValue: TOnGetSetCheckValue read FOnSetValue write FOnSetValue;
  end;
...
begin
  ...
  begin
    if MultiFields then
      ...
      begin 
        try
          FieldList.Add(GetFieldProperty(DataSource.DataSet, FControl, FFieldName))
        except
          if FControl is TWinControl then
            TWinControl(FControl).Enabled := False;
        end;
      end
      else begin
        if DataSource.DataSet.FindField(FFieldName) <> nil then
          FieldList.Add(DataSource.DataSet.FieldByName(FFieldName));
      end;
  end;
  SetField(FieldList);
  FieldList.Free;
...
{$endif}
  ...
  FDisableButtonWhenReadOnly := False;
...
begin
  ...
  {#7}
  if Result then
    Result := not (DisableButtonWhenReadOnly and (not FDataLink.CanModify {or ReadOnly}));
end;
...
begin
  ...
  begin
    ...
    if (Key = VK_DOWN) and (Shift = [ssAlt]) and
       MRUList.Active and Showing and not FDroppedDown
    then
      MRUList.DropDown;
  end;
...
var
  s :String;
  i :Integer;
  IllegalChars : Set of AnsiChar;
begin
  s := inherited Text;
  IllegalChars := [];
  if not WantTabs then                     //недопустимы ^I
    IllegalChars := IllegalChars + [^I];
  if not WantReturns  then                 //недопустимы ^M и ^J
  begin
    IllegalChars := IllegalChars + [^M,^J];
    s := AnsiReplaceStr(s,^J,^M);        //^M^J -> ^M - пара ^M^J
  end;                                     //соответствует одному переводу строки

  if IllegalChars <> [] then
  begin
    // удаление завершающих символов
    i := Length(s);
    while (i > 0) and (s[i] in IllegalChars) do
      Dec(i);
    SetLength(s,i);
    // замена внутренних символов на пробел
    while (i > 0) do
    begin
      if s[i] in IllegalChars then
        s[i] := ' ';
      Dec(i);
    end;
  end;

  if s <> inherited Text then  inherited Text := s;
  FDataLink.Modified;
  Modified := True;
  inherited Change;
  UpdateImageIndex();
...
begin
  ...
  begin
    FReadOnly := Value;
    EditingChanged;
    if DisableButtonWhenReadOnly then
      UpdateEditButtonControlsState;
  end;
end;
...
var NewEditButtonControlsCount, OldEditButtonControlsCount: Integer;
  ...
  begin
    ...
    ControlRec.EditButtonControl.Enabled := Button.Enabled; {#6}
...
procedure TCustomDBEditEh.UpdateEditButtonControlsState;
  {#7}
  function EditButtonEnabled(AValue : Boolean) : Boolean;
  begin
    Result := False;
    if AValue then
      Result := not (DisableButtonWhenReadOnly and (not FDataLink.CanModify {or ReadOnly}));
...
begin
  ...
  if not Enabled then  {#6}
    FEditButtonControlList[0].EditButtonControl.Enabled := ButtonEnabled
  else
    FEditButtonControlList[0].EditButtonControl.Enabled:= EditButtonEnabled(EditButton.Enabled);
  ...
  for i := 1 to Length(FEditButtonControlList) - 1 {EditButtons.Count} do
    ...
    begin
      if not Enabled then {#6}
        ...
        FEditButtonControlList[i].EditButtonControl.Enabled:= EditButtonEnabled(EditButtons[i-1].Enabled);
...
procedure TCustomDBEditEh.SetDisableButtonWhenReadOnly(
  const Value: Boolean);
begin
  if FDisableButtonWhenReadOnly <> Value then
  begin
    FDisableButtonWhenReadOnly := Value;
    UpdateEditButtonControlsState;
  end;  
end;
...
function CorrectDate(var Year, Month, Day: Integer; const AFirstDay : Boolean = False): Boolean;
var
  CurYear, CurMonth, CurDay: Word;
begin
  Result := False;
  DecodeDate(Date, CurYear, CurMonth, CurDay);
  if (Day <= 0) or AFirstDay then
  begin
    if not AFirstDay then
      Day := CurDay
    else
      Day := 1;
  end;
...
begin
  ...
  begin
    with DateTimeMaskPos do
      AFirstDay := Year.Present and Month.Present and not Day.Present;

    with DateTimeStamp do
    begin
      CorrectDate(Year, Month, Day, AFirstDay);
...
begin
{$IFDEF EH_LIB_6}
  if FExtItemsEh <> nil then
    FreeAndNil(FExtItemsEh);
{$ENDIF}
  FDropDownBox.Free;
  FKeyItems.Free;
  FItems.Free;
...
procedure TCustomDBComboBoxEh.SetOnGetItemsListEh(const Value : TGetItemsListEh);
begin
  FOnGetItemsListEh := Value;
{$IFDEF EH_LIB_6}
  if Assigned(FExtItemsEh) then
    FreeAndNil(FExtItemsEh);
  if Assigned(FOnGetItemsListEh) then
    FExtItemsEh := TStringListEh.Create;
{$ENDIF}
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if not Assigned(Self.OnGetItemsListEh) then
      begin
{$IFDEF EH_LIB_6}
        ExtItems := Self.Items;
...
{$IFDEF EH_LIB_6}
        ExtItems := FExtItemsEh;
        ExtItems.Assign(Self.Items);
        Count := ExtItems.Count;
        Self.OnGetItemsListEh(Self, ExtItems);
{$ELSE}
        Items.Assign(Self.Items);
        Self.OnGetItemsListEh(Self, Items);
{$ENDIF}
      end;
...
function TCustomDBComboBoxEh.ButtonEnabled: Boolean;
begin
  Result := inherited ButtonEnabled;
  if Result then
    Result := FEditButton.Enabled;
end;
...
begin
  if DisplayFormat <> '' then
    ...
    Result := FloatToStrF(Value, ffFixed, 20, DecimalPlaces); {#1} //FloatToStr(Value);
end;
...
begin
  ...
  begin
    ...
    FDataLink.Modified;
    if FFocused and FDataLink.CanModify then
      ReformatEditText(FloatToStrF(FValue, ffFixed, 20, DecimalPlaces)) {#2}//FormatFloat(FEditFormat, FValue))
...
procedure TCustomDBNumberEditEh.ReCheckValue;
begin
