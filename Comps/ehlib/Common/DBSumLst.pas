...
type
  ...
  TProducerDBSumEvent = procedure(Sender : TObject; var Handled : Boolean) of object;
  ...
  public
    function GetSumByOpAndFName(AGroupOperation: TGroupOperation; AFieldName: String;
      Column : TObject; FooterNo : integer): TDBSum;
  ...
  protected
    ...
    FOnProducerDBSum : TProducerDBSumEvent;
    FOwner: TComponent;
    ...
    OldAfterRefresh: TDataSetNotifyEvent;
  ...
  public
    ...
    property OnProducerDBSum : TProducerDBSumEvent read FOnProducerDBSum write FOnProducerDBSum;
  end;
...
begin
  ...
  begin
    ...
    OldAfterRefresh := FDataSet.AfterRefresh;
    if Assigned(FMasterDataSet)
      then OldMasterAfterScroll := FMasterDataSet.AfterScroll;
    ...
    FDataSet.AfterRefresh := DataSetAfterRefresh;
    if Assigned(FMasterDataSet)
      then FMasterDataSet.AfterScroll := MasterDataSetAfterScroll;
...
begin
  ...
  begin // Return old events
    ...
    FDataSet.AfterRefresh := OldAfterRefresh;
      ...
      try
        ...
        begin
          ...
          begin
            ...
            if (item.GroupOperation = goCount) or ((item.FieldName <> '') and
               (FDataSet.FindField(item.FieldName) <> nil))
            then
            begin
              Handled := True;
              if Assigned(OnProducerDBSum) then
                OnProducerDBSum(item,Handled);
              if Handled then
...
procedure TDBSumListProducer.DataSetAfterRefresh(DataSet: TDataSet);
begin
  if Active then RecalcAll;
  if (Assigned(OldAfterRefresh)) and (@OldAfterRefresh <> @DataSet.AfterRefresh) then
...
begin
  ...
  begin
    ...
    begin
      item := TDBSum(FSumCollection.Items[i]);
      if (item.GroupOperation = goCount) or
         ((item.FieldName <> '') and (FDataSet.FindField(item.FieldName)<> nil))
      then begin
        Handled := True;
        if Assigned(OnProducerDBSum) then
          OnProducerDBSum(item,Handled);
...
function TDBSumCollection.GetSumByOpAndFName(
  AGroupOperation: TGroupOperation; AFieldName: String;
  Column : TObject; FooterNo : integer): TDBSum;
var i: Integer;
begin
  Result := nil;
  ...
  begin
    ...
    begin
      if not Assigned(Column) or
        ((Items[i].Column = Column) and (Items[i].FooterNo = FooterNo))
      then begin
        Result := Items[i];
        Exit;
      end;
